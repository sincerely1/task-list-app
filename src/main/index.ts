...
app.whenReady().then(() => {
  ...
  ipcMain.handle('closeDatePick', function (event) {//关闭DatePick窗口
  ...
  ipcMain.handle('showDatePick', function (event) {
    ...
    let x: number = (positon[0] - 500) > 0 ? positon[0] - 500 : 100
    let y: number = (positon[1] - 750) > 0 ? positon[1] - 750 : 100
    const win_config = {
      x: x,
      y: y,
      width: 500,
    ...
    let datePickWindow = new BrowserWindow(win_config)
    ...
    datePickWindow.on('close', () => {//关闭时消除数据
    ...
    if (is.dev && process.env['ELECTRON_RENDERER_URL']) {
      ...
      datePickWindow.loadFile(join(__dirname, '../renderer/index.html/datePick'))
    }
    datePickWindow.show()
    return true
  })
  ...
  ipcMain.handle('showSetting', function (event) {
    ...
    let settingWindow = new BrowserWindow(win_config)
    ...
    if (is.dev && process.env['ELECTRON_RENDERER_URL']) {
      ...
      settingWindow.loadFile(join(__dirname, '../renderer/index.html/setting'))
    }
    settingWindow.show()
    settingWindow.on('close', () => {//关闭时消除数据
      BrowserWindowsMap.delete(settingWindowId)
      settingWindowId = 0
    })
    return true
